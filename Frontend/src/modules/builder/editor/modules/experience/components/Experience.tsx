...
interface IExperienceProps {
  experienceInfo: IExperienceItem;
  currentIndex: number;
  onExperienceChange: (index: number, name: string, value: any) => void;
}

const Experience: React.FC<IExperienceProps> = ({ experienceInfo, currentIndex, onExperienceChange }) => {
  const handleChange = useCallback(
    (name: string, value: any) => {
      onExperienceChange(currentIndex, name, value); // Call the parent handler with updated values
    },
    [currentIndex, onExperienceChange]
  );

  const onSummaryChange = useCallback(
    (htmlOutput: string) => {
      handleChange('summary', htmlOutput); // Handle summary change
    },
    [handleChange]
  );
  ...
  return (
    <Fragment>
      <TextField
        ...
        variant="filled"
        value={experienceInfo.name}
        onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange('name', e.target.value)} // Update name
        autoComplete="off"
        fullWidth
        required
        autoFocus={true}
      ...
      <TextField
        ...
        variant="filled"
        value={experienceInfo.position}
        onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange('position', e.target.value)} // Update position
        autoComplete="off"
        fullWidth
        required
        sx={{ marginBottom: '26px' }}
      ...
      <DatePicker
        label="Start date"
        value={experienceInfo.startDate}
        onChange={(newDate) => handleChange('startDate', newDate)} // Update startDate
        inputFormat={DATE_PICKER_FORMAT}
        renderInput={(params) => (
          <TextField {...params} variant="filled" autoComplete="off" fullWidth required />
        )}
      ...
      <SwitchWidget
        label={'I currently work here'}
        value={experienceInfo.isWorkingHere ?? false}
        onChange={(newValue: boolean) => handleChange('isWorkingHere', newValue)} // Update isWorkingHere
      />
      <DatePicker
        label="End date"
        value={experienceInfo.isWorkingHere ? null : experienceInfo.endDate}
        onChange={(newDate) => handleChange('endDate', newDate)} // Update endDate
        inputFormat={DATE_PICKER_FORMAT}
        renderInput={(params) => (
          <TextField
            {...params}
      ...
      <TextField
        ...
        variant="filled"
        value={experienceInfo.years}
        onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange('years', e.target.value)} // Update years
        autoComplete="off"
        fullWidth
        sx={{ marginBottom: '26px' }}
      />
      <RichtextEditor
        label="Few points on this work experience"
        value={experienceInfo.summary}
        onChange={onSummaryChange} // Update summary
        name="summary"
      />
    </Fragment>
  );
