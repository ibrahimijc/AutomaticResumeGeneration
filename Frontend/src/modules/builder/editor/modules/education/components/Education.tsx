...
//   console.log('educationInfoc', educationInfo);
...
interface IEducationProps {
  educationInfo: IEducationItem;
  currentIndex: number;
  onEducationChange: (index: number, name: string, value: any) => void;
}




const Education: React.FC<IEducationProps> = ({ educationInfo, currentIndex, onEducationChange }) => {
  const handleChange = useCallback(
    (name: string, value: any) => {
      onEducationChange(currentIndex, name, value);
    },
    [currentIndex, onEducationChange]
  );
  ...
  return (
    <Fragment>
      ...
      <TextField
        ...
        variant="filled"
        value={educationInfo.studyType}
        onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange('studyType', e.target.value)}
        autoComplete="off"
        fullWidth
        required
        sx={{ marginBottom: '26px' }}
      ...
      <TextField
        ...
        variant="filled"
        value={educationInfo.score}
        onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange('score', e.target.value)}
        autoComplete="off"
        fullWidth
        required
        sx={{ marginBottom: '26px' }}
      ...
      <SwitchWidget
        label={'I currently study here'}
        value={educationInfo.isStudyingHere ?? false}
        onChange={(newValue: boolean) => handleChange('isStudyingHere', newValue)}
      />
      <DatePicker
        label="End date"
        value={educationInfo.isStudyingHere ? null : educationInfo.endDate}
