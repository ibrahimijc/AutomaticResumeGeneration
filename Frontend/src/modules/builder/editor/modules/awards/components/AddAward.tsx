...
const AddAward = ({
  ...
  handleChange: (name: string, isExpanded: boolean) => void;
  isEmpty: boolean;
  setAwardsData: React.Dispatch<React.SetStateAction<IAwardItem[]>>;
}) => {
  const onCreateAward = () => {
    const uniqueExpandedId = `${Math.random()}`;
    NEW_AWARD.id = uniqueExpandedId;
    setAwardsData(prevData => [...prevData, NEW_AWARD]);
    handleChange(uniqueExpandedId, true);
  };

  const buttonCaption = useMemo(() => {
    return isEmpty ? '+ Add an award' : '+ Add more';
  }, [isEmpty]);

  return (
    <div className="flex gap-2 mt-3">
      <OutlinedButton onClick={onCreateAward} disabled={false}>
        {buttonCaption}
      </OutlinedButton>
    </div>
  );
